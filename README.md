...
It also ships `Sessions/Topics.base` — a filterable, groupable table view built on Obsidian's native Bases feature (no plugin), reading the same ficha frontmatter as the plain table in `Sessions/_index.md`. Open it from Obsidian to filter by status or group by project interactively instead of scanning the static table.
